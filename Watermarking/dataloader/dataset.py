...
class attrsImgDataset(Dataset):
    def __init__(self, path, image_size, attr_path="celeba"):
        ...
        if attr_path[0: len("celebahq")] != "celebahq":
            self.attr_path = "E:/scpcode/WaveGuard-master1/network/noise/stargan/list_attr_celeba.txt"
        else:
            self.attr_path = "E:/scpcode/WaveGuard-master1/network/noise/stargan/CelebAMask-HQ-attribute-anno.txt"
    ...
    def preprocess(self):
        ...
        for i, line in enumerate(lines):
            ...
            if os.path.exists(
                    os.path.join(self.image_dir, str(filename).zfill(5) + ".jpg")
            ):
                self.list.append([str(filename).zfill(5) + ".jpg", label])
