...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # print("cfg.message_length:", cfg.message_length)  # 应该是 30
        ...
        self.down = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Conv2d(cfg.wm_channels, 3, kernel_size=3, padding=1),
        )
        ...
        for linear_layer in [self.linear0, self.linear1, self.linear2]:
            ...
            # print('Linear layer:', linear_layer)  # 打印当前 linear_layer
        for m in self.modules():
    ...
    def forward(self, x, watermark):
        # print("watermark shape1111111111111:", x.size(),watermark.shape)  # 打印 watermark 的形状
        fm = self.feature_extractor(x)
        fm = self.attention(fm)
        #print('sada', fm.size(), fm.size())
        wm_ex_0 = self.linear0(watermark)
        # print("wm_ex_0 shape after linear0:", wm_ex_0.shape)  # 打印线性层后的形状
        wm_ex_0 = wm_ex_0.view(-1, 1, cfg.message_length, cfg.message_length)
        # print("wm_ex_0 shape after view:", wm_ex_0.shape)  # 打印重塑后的形状
        ...
        return x + cfg.wm_factor * wm_diff
